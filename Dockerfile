...
RUN GPG_KEYS=B0F4253373F8F6F510D42178520A9993A1C052F8 \
	...
	&& ln -sf /dev/stderr /var/log/nginx/error.log \
        ...
        && apk add --no-cache --virtual .htpasswd apache2-utils \
	&& mv /usr/bin/htpasswd /tmp/ \
	&& apk del .htpasswd \
	&& mv /tmp/htpasswd /usr/bin/ \
	&& apk add --no-cache apr apr-util \
        && apk --no-cache add tini \
        && mkdir -p /log /etc/certs.d/ && mkdir -p /etc/nginx/conf.d/

ENV WORKER_USERNAME=nginx

COPY nginx.conf.templ /etc/nginx/nginx.conf.templ
COPY nginx.ldap.conf.templ /etc/nginx/conf.d/nginx.ldap.conf.templ
COPY bad.* /etc/certs.d/
COPY docker-entrypoint.sh /

STOPSIGNAL SIGTERM

VOLUME ["/log", "/etc/certs.d"]
