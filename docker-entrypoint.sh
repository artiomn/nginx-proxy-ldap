...
export MIN_DELETE_DEPTH=${MIN_DELETE_DEPTH:-0}
...
envsubst '${SSL} ${CERTIFICATE} ${CERTIFICATE_KEY} ${LISTFORMAT} ${LDAP_PORT} ${LDAP_PROTOCOL} ${LDAP_DN} ${LDAP_SERVER} ${LDAP_DOMAIN} ${LDAP_BIND_USER} ${LDAP_BIND_PASSWORD} ${LDAP_FILTER} ${MIN_DELETE_DEPTH} ${LDAP_AUTH_MESSAGE} ${LDAP_OPEN_METHODS} ${LDAP_OPEN_METHODS_END} ${TRUSTED_SUBNET} ${LISTENPORT} ${PROXY_ENDPOINT}' > /etc/nginx/conf.d/default.conf < /etc/nginx/conf.d/${SOURCE_TEMPLATE}
